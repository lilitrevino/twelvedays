...
# Test Your Function
```{r, echo=FALSE}
 make_phrase(xmas$Day, xmas$Day.in.Words,xmas$Gift.Item, xmas$Verb, xmas$Adjective, xmas$Location)
```
...
to say `"twelve"` **not** `"twelfth"`. I suggest using the **english** package to create numbered days and then use those in your `pmap_chr()`!
 
```{r, echo=FALSE}
twelve_nums <- english(1:12)
more_nums <- as.character(twelve_nums)


xmas <- xmas %>% 
  mutate(
    Full.Phrase = pmap_chr(list(Day, more_nums, Gift.Item, Verb, Adjective, Location), make_phrase))
...
Run the following code to tet out your functions! The o should be the 
